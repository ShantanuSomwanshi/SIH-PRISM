...
STANDARD_RE = re.compile(
    ...
    r"(\d{1,5})"
    r"(?:\s*\(?\s*Part\s*[-–]?\s*([0-9]{1,3}|[IVXivx]{1,5}))?"
    r"(?:\s*[/,]?\s*Sec(?:tion)?\s*[-–]?\s*([0-9]{1,4}|[IVXivx]{1,5}))?"
    r"\s*\)?"
    r"(?:\s*[:\-]\s*((?:19|20)\d{2}))?",
    re.IGNORECASE,
)
...
BIS_SCHEME_RE = re.compile(r"Under\s+BIS\s+Scheme\s*[-–]?\s*([IVX]+)", re.IGNORECASE)
ISI_RE = re.compile(r"ISI\s*[-–]?\s*Marked", re.IGNORECASE)

LINK_PHRASES = re.compile(
    r"\b(conforming\s+to|confirming\s+to|as\s+per|ISI\s*[-–]?\s*marked\s+to|"
...
TRAILING_CONNECTOR = re.compile(
    r"[\s,;:\-–]*\b(to|per|as|and|or|conforming|confirming|marked|of|for)\b\s*$",
    re.IGNORECASE,
)
